...
public class Heladeria extends JFrame implements ActionListener {
	...
	protected void actionPerformedBtnProcesar(ActionEvent arg0) {
		...
		int caramelos = calcularCaramelos(cant);
		int gomas = calcularGomitasMascar(marca,cant);
		mostrarResultados(impP,caramelos,gomas);
	}
	int leerCantidad(){
	...
	int calcularCaramelos(int cant){
		if(cant<10){ return 5;}
		else if(cant>=10 && cant<20){ return 10;}
		else if(cant>=20 && cant<30){ return 15;}
		else{return 20; }
	}
	int calcularGomitasMascar(String marca, int cant){
		switch(marca){
		case "Sol": return 1*cant;
		case "Mar": return 2*cant;
		default: return 6;
	}
	}
	void mostrarResultados(double impP,int caramelos,int gomas) {
		txtS.setText("");
		imprimir("Importe a pagar : " + impP);
		imprimir("Caramelos       : " + caramelos);
		imprimir("Gomitas         : " + gomas);
	}
}